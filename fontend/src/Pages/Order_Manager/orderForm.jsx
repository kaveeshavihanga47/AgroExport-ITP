...
const OrderForm = () => {
    ...
    const handleChange = (e) => {
        ...
        // Validation for amount and shippingfee fields: only positive numbers and no letters
        if (name === 'amount' || name === 'shippingfee') {
            const regex = /^[0-9]*\.?[0-9]*$/; // Allows numbers and decimals

            if (!regex.test(value)) {
                setErrors((prev) => ({
                    ...prev,
                    [name]: `${name.charAt(0).toUpperCase() + name.slice(1)} must be a valid number`
                }));
                ...
                setErrors((prev) => ({
                    ...prev,
                    [name]: undefined // Clear error if valid
                }));
    ...
    useEffect(() => {
        ...
        setFormData((prev) => ({ ...prev, totalprice }));
    }, [formData.amount, formData.shippingfee]);
    ...
    return (
        <form 
            ...
            <h2 style={{ textAlign: 'center', color: '#333' }}>Add New Order</h2>
